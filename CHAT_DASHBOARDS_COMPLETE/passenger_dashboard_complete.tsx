...
export default function PassengerDashboard() {
  ...
  const loadUnreadMessageCount = async () => {
    try {
      const token = await AsyncStorage.getItem('access_token');
      const response = await axios.get(`${API_URL}/api/users/messages/unread`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setUnreadMessageCount(response.data.unread_count);
    } catch (error) {
      console.log('Error loading unread message count:', error);
    }
  };
  ...
  const checkForCompletedTrips = async () => {
    try {
      const token = await AsyncStorage.getItem('access_token');
      const response = await axios.get(`${API_URL}/api/trips/my`, {
        headers: { Authorization: `Bearer ${token}` },
      ...
      const justCompleted = response.data.find(
        (trip: Trip) => trip.status === 'completed' && !localStorage.getItem(`rated_${trip.id}`)
      );

      if (justCompleted && justCompleted.driver_id) {
        setCompletedTrip(justCompleted);
        setShowRatingModal(true);
      }
    } catch (error) {
      console.log('Error checking completed trips:', error);
    }
  };
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      localStorage.setItem(`rated_${completedTrip.id}`, 'true');
