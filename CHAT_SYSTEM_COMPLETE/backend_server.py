...
async def send_message_to_user(user_id: str, message_data: AdminMessageCreate, current_user: User = Depends(get_current_user)):
    """Admin sends direct message to a user"""
    if current_user.user_type != UserType.ADMIN:
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    # Verify the user_id in the message data matches the URL parameter
...
@api_router.get("/users/messages")
async def get_user_messages(current_user: User = Depends(get_current_user)):
    """Get messages sent to current user by admin"""
    messages = await db.admin_messages.find({"user_id": current_user.id}).sort("created_at", -1).to_list(100)
    
    # Mark all messages as read when fetched
    await db.admin_messages.update_many(
        {"user_id": current_user.id, "read": False},
        {"$set": {"read": True}}
    )
    
    return [AdminMessageToUser(**message) for message in messages]

@api_router.get("/users/messages/unread")
async def get_unread_message_count(current_user: User = Depends(get_current_user)):
    """Get count of unread messages for current user"""
    count = await db.admin_messages.count_documents({
        "user_id": current_user.id,
        "read": False
    })
    return {"unread_count": count}

# Bulk delete endpoints for admin
...
async def bulk_delete_users(request: BulkDeleteRequest, current_user: User = Depends(get_current_user)):
    ...
    # Delete users
    result = await db.users.delete_many({"id": {"$in": request.ids}})
...
async def bulk_delete_trips(request: BulkDeleteRequest, current_user: User = Depends(get_current_user)):
    ...
    return {
        "message": f"Successfully deleted {result.deleted_count} trips",
        "deleted_count": result.deleted_count
    }
